...
class UpdatePermissionForm extends StatefulWidget {
  const UpdatePermissionForm(
      {super.key, required this.getPermissionOfSpecificEmployeeResponse});
...
class _UpdatePermissionFormState extends State<UpdatePermissionForm> {
  ...
  void initState() {
    ...
    context.read<UpdatePermissionCubit>().startTimeController.text =
        widget.getPermissionOfSpecificEmployeeResponse.start.toString();
    context.read<UpdatePermissionCubit>().endTimeController.text =
        widget.getPermissionOfSpecificEmployeeResponse.end.toString();
    context.read<UpdatePermissionCubit>().dateController.text =
        widget.getPermissionOfSpecificEmployeeResponse.date.toString();
    context.read<UpdatePermissionCubit>().descriptionController.text =
        widget.getPermissionOfSpecificEmployeeResponse.description.toString();
  }

  @override
  Widget build(BuildContext context) {
    return Form(
      key: context.read<UpdatePermissionCubit>().formKey,
      child: Column(
        ...
        children: [
          ...
          AppTextFormField(
            controller:
                context.read<UpdatePermissionCubit>().startTimeController,
            ...
            suffixIcon: IconButton(
              onPressed: () async {
                ...
                if (time != null) {
                  // Checking if the user selected a time or cancelled
                  final formattedTime = formatTimeOfDay(time);
                  setState(() {
                    context
                        .read<UpdatePermissionCubit>()
                        .startTimeController
                        .text = formattedTime;
                  });
                }
          ...
          AppTextFormField(
            ...
            controller:
                context.read<UpdatePermissionCubit>().descriptionController,
          ),
        ],
      ),
    );
